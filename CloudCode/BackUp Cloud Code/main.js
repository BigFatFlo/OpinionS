...
////// I - Users: beforeSave, afterSave, usersAround, addSubscriber, subtractSubscriber and usersLocation
...
////// I - Users: beforeSave, afterSave, usersAround, addSubscriber, subtractSubscriber, usersLocation and userExists
				...
				return channelQuery.first().then(function(resultChannel) { // Querying the first matching, unfull, channel
					...
					resultChannel.set("maxMembers",maxMembers); // Resetting, or updating the maximum number of members. Makes changing the number easier.
						if (m + 1 == maxMembers) {
							...
							resultCountry.set("avUsersPerChannel",Math.min(maxMembers,Math.floor((nbrUsers+1)/(nbrChannels+1)))); // Updating "country" International's average number of users per channel
							} else {
							resultCountry.set("avUsersPerChannel",Math.min(maxMembers,Math.floor((nbrUsers+1)/nbrChannels))); // Updating "country" International's average number of users per channel
							}	
						} else {
							...
							resultCountry.set("avUsersPerChannel",Math.min(maxMembers,Math.floor((nbrUsers+1)/nbrChannels)));
							}
						}
					...
					resultCountry.set("avUsersPerChannel",Math.min(maxMembers,Math.floor((nbrUsers+1)/nbrChannels))); // Updating average number of users per channel
		...
		var oldInternationalChannelName = request.object.get("internationalChannel"); // We need to update the number of members of the "country" International and of the user's former international channel
			...
			oldInternationalChannelQuery.first().then(function(oldInternationalChannel) {
				...
				return query.first().then(function(resultCountry) {
					...
					resultCountry.set("avUsersPerChannel",Math.min(maxMembers,Math.floor((internationalUsers-1)/internationalChannels))); // No need to lower the nbrChannels for International
		...
		if (oldChannelName != null) { // If it's not the first time the user chooses his country (ie) he already has a channel
			...
			oldChannelQuery.first().then(function(oldChannel) {
				...
				oldCountryQuery.first().then(function(oldCountry) {
					...
					oldCountry.set("avUsersPerChannel",Math.min(maxMembers,Math.floor((oldCountryUsers-1)/oldCountryChannels))); // No need to lower the nbrChannels for the country
																												// the channel will be filled again at the next subscription
					}
				...
				return channelQuery.first().then(function(resultChannel) { // Querying the first matching, unfull, channel
					...
					resultChannel.set("maxMembers",maxMembers); // Resetting, or updating the maximum number of members. Makes changing the number easier.
						if (m + 1 >= maxMembers) {
							...
							resultCountry.set("avUsersPerChannel",Math.min(maxMembers,Math.floor((nbrUsers+1)/(nbrChannels+1))));
					...
					resultCountry.increment("nbrUsers");
					resultCountry.set("avUsersPerChannel",Math.min(maxMembers,Math.floor((nbrUsers+1)/nbrChannels))); // Updating country (no new channel because it has never
															  														  // been full yet
...
Parse.Cloud.afterSave("Question", function(request){
	...
	pushQuery.find().then(function(results) { // If the Installations have been found succesfully
		...
		pushQuery.limit(10); // Limiting to a certain number of users since it is only a notification for approval
	...
	//pushQuery.skip(skip-1); // Keeping a random sample of these installations to which the request for approval is to be sent: skipping the first "skip" installations
	//pushQuery.limit(range); // And taking only the "range" next ones
	Parse.Push.send({
	...
	Parse.Push.send({
		...
		data: {
			...
			questionID: request.object.id,
			creationDate: request.object.createdAt,
			questionText: request.object.get("text"),
			nbrAnswers: request.object.get("nbrAnswers"),
